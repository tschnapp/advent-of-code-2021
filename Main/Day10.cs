    ...
    class Day10 : Day
    {
        ...
        public override string Part01(string[] rawInput)
        {
            ...
            foreach (string error in input)
            {
                ...
                {
                    if ("([{<".Contains(c.ToString()))                                          // opening char
                    {
                        stack.Add(c.ToString());
                    }
                    else if (")]}>".Contains(c.ToString()))                                     // closing char
                    {
                        if ("([{<".IndexOf(stack.Last()) != ")]}>".IndexOf(c))                  // bad char
                        {
                            subTotal += new int[] { 3, 57, 1197, 25137 }[")]}>".IndexOf(c)];    // add to total
                            break;
                        }
        ...
        public override string Part02(string[] rawInput)
        {
            ...
            foreach (string error in input)
            {
                ...
                foreach (char c in error)
                {
                    ...
                    {
                        if ("([{<".IndexOf(stack.Last()) != ")]}>".IndexOf(c))                  // bad char, skip this one
                        {
                            isCorrupt = true;
                            break;
                        }
                ...
                if (!isCorrupt)
                {
                    ...
                    {
                        subTotal = (subTotal * 5) + "([{<".IndexOf(stack[c]) + 1;               // get sub total
                    }
